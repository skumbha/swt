...
public class TestCourser extends Dialog {
	private Table table;
	 ...
	 * @param parentShell
	 */
	public TestCourser(Shell parentShell) {
		super(parentShell);
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		final TableViewer tableViewer = new TableViewer(composite, SWT.BORDER| SWT.FULL_SELECTION | SWT.HIDE_SELECTION);
		...
		tableCursor.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if(ctrlKeyPressed && !shiftKeyPressed){
					if(e.keyCode == SWT.ARROW_RIGHT){
						tableCursor.setSelection(table.indexOf(tableCursor.getRow()), tableViewer.getTable().getColumnCount()-1);
						return;	
					}
					...
					if(e.keyCode == SWT.ARROW_DOWN){
						tableCursor.setSelection(tableViewer.getTable().getItemCount()-1, tableCursor.getColumn());
						return;	
					}
				}
			...
			private void selectCellsOnArrowKeys(final TableViewer tableViewer, KeyEvent e) {
				if((e.stateMask & SWT.SHIFT) != 0){	        		
					...
					List<Point> cellsTobeSelected = getAllCellsBetweenTwoCells(selectionStartPoint,selectionEndPoint,tableViewer);
	...
	private void clearSelection(List<Point> currentSelection,TableViewer tableViewer){
    	for(Point cell: currentSelection){
			tableViewer.getTable().getItem(cell.x).setBackground(cell.y, new Color(null, 255, 255, 255));
		}
	...
	private void highlightCells(List<Point> cellsTobeHighlight,TableViewer tableViewer) {
		for(Point cell : cellsTobeHighlight){
			tableViewer.getTable().getItem(cell.x).setBackground(cell.y, new Color(null, 255, 204, 102));	
		}
	}
	...
	private List<Point> getAllCellsBetweenTwoCells(Point selectionStartPoint,Point selectionEndPoint,TableViewer tableViewer){
	...
	public static void main(String[] args) {
		TestCourser testCourser = new TestCourser(new Shell());
		testCourser.open();
	}
}
